...
from database import engine, Base_db, get_db
...
# Rota de cadastros de cursos
# A função é antecedida pelo DECORATOR @app.post que informa o verbo HTTP POST
...
# Função get_all irá processar as requisições feitas para Rota /api/cursos
...
# Função find_by_id irá tratar as pesquisas por "id" na Rota /api/cursos/{id} com verbo HTTP POST
@app.get("/api/cursos/{id}", response_model=CursoResponse)
def find_by_id(id: int, db: Session = Depends(get_db)):
    curso = CursoRepository.find_by_id(db, id)
    if not curso:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Find: Curso não encontrado"
        )
    return CursoResponse.from_orm(curso)

# Rota de exclusão de curso por id
# A função é antecedida pelo DECORATOR @app.delete que informa o verbo HTTP DELETE
@app.delete("/api/cursos/{id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_by_id(id: int, db: Session = Depends(get_db)):
    if not CursoRepository.exists_by_id(db, id):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Delete: Curso não encontrado"
        )
    CursoRepository.delete_by_id(db, id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)

# Rota de atualização de curso
# A função é antecedida pelo DECORATOR @app.put que informa o verbo HTTP PUT
@app.put("/api/cursos/{id}", response_model=CursoResponse)
def update(id: int, request: CursoRequest, db: Session = Depends(get_db)):
    if not CursoRepository.exists_by_id(db, id):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="UpDate: Curso não encontrado"
        )
